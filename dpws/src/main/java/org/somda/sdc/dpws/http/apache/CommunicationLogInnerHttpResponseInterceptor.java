 ...
 * Inner Part - called after the Response has been decompressed.
 */
public class CommunicationLogInnerHttpResponseInterceptor implements HttpResponseInterceptor {
    private static final Logger LOG = LogManager.getLogger(CommunicationLogInnerHttpResponseInterceptor.class);

    private final CommunicationLog commlog;
    private final Logger instanceLogger;

    CommunicationLogInnerHttpResponseInterceptor(CommunicationLog communicationLog, String frameworkIdentifier) {
    ...
    public void process(HttpResponse response, HttpContext context) {
        ...
        if (extractingEntity != null) {
            OutputStream commLogNetLevelStream = commlog.logRelatedMessage(
                commLogAppLevelStream,
                CommunicationLog.Direction.INBOUND,
                CommunicationLog.TransportType.HTTP,
                CommunicationLog.MessageType.RESPONSE,
